...
class PullRequestDeploymentRootResourceTest {
  ...
  void setup() {
    ...
    PullRequestDeploymentRootResource resource = new PullRequestDeploymentRootResource(
  ...
  @Nested
  class GetPullRequestDeployments {
    ...
    void setup() {
      ...
      deployments.put(buildDefaultDeployment(DeploymentType.PULL_REQUEST));
      lenient().when(deploymentMerger.mergePullRequestDeployments(repository, "1337")).thenReturn(deployments);
    ...
    void shouldThrowNotFoundBecauseRepositoryIsUnknown() throws URISyntaxException {
      ...
      MockHttpRequest request = MockHttpRequest.get("/v2/deployments/unknown-namespace/unknown-name/pull-requests/1337");
      MockHttpResponse response = new MockHttpResponse();
      dispatcher.invoke(request, response);

      assertThat(response.getStatus()).isEqualTo(404);
      verifyNoInteractions(deploymentService);
      verifyNoInteractions(deploymentMerger);
    }
    ...
    void shouldAddWriteLinks() throws URISyntaxException {
      ...
      assertThat(responseBody.get(0).get("_links").get("delete").get("href").asText()).isEqualTo(
        String.format("%sv2/deployments/%s/%s/pull-requests/1337/ArgoCD/ARM64+Linux", domainForLinks, repository.getNamespace(), repository.getName())
      );
      assertThat(responseBody.get(0).get("_links").get("update").get("href").asText()).isEqualTo(
        String.format("%sv2/deployments/%s/%s/pull-requests/1337", domainForLinks, repository.getNamespace(), repository.getName())
      );
    }
  }
  ...
  class DeleteDeployment {
    ...
    void shouldThrowNotFoundBecauseRepositoryIsUnknown() throws URISyntaxException {
      MockHttpRequest request = MockHttpRequest.delete(
        "/v2/deployments/unknown-namespace/unknown-name/pull-requests/1337/ArgoCD/ARM64+Linux"
      );
      MockHttpResponse response = new MockHttpResponse();
      dispatcher.invoke(request, response);
    ...
    void shouldDeleteDeployment() throws URISyntaxException {
      ...
      MockHttpRequest request = MockHttpRequest.delete(
        String.format(
          "/v2/deployments/%s/%s/pull-requests/1337/ArgoCD/ARM64+Linux",
      ...
      verify(deploymentService).deletePullRequestDeployment(
  ...
  class PutDeployment {
    ...
    void shouldThrowNotFoundBecauseRepositoryIsUnknown() throws URISyntaxException {
      MockHttpRequest request = MockHttpRequest.put(
          "/v2/deployments/unknown-namespace/unknown-name/pull-requests/1337"
    ...
    @Test
    void shouldUpdateDeploymentStatus() throws URISyntaxException {
      when(repositoryManager.get(any(NamespaceAndName.class))).thenReturn(repository);
      ...
      assertThat(response.getStatus()).isEqualTo(204);
      verify(deploymentService).putPullRequestDeployment(
        buildDefaultDeployment(DeploymentType.PULL_REQUEST),
        repository,
        "1337"
      );
    }
